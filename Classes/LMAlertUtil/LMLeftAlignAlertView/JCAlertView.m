...
- (void)viewDidLoad{
    ...
    [self addCoverView];  // 添加黑色模态透明背景
    [self addAlertView];  // 弹出自定义弹出框视图
}
...
- (void)setup{
    ...
    if (self.isCustomAlert) {  // 判断是否自定义弹出框视图
        return;
    }
...
- (void)setButton:(UIButton *)btn BackgroundWithButonType:(JCAlertViewButtonType)buttonType{
    ...
    switch (buttonType) {
        case JCAlertViewButtonTypeDefault:
            ...
            textColor = [[UIColor blueColor] colorWithAlphaComponent:0.6];
