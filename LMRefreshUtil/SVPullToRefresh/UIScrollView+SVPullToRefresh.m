...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.pullToRefreshView) {
        ...
        SVPullToRefreshView *view = [[SVPullToRefreshView alloc] initWithFrame:CGRectMake(0, -SVPullToRefreshViewHeight, DEVICE_SCREEN_WIDTH, SVPullToRefreshViewHeight)];
...
- (SVPullToRefreshArrow *)arrow {
    ...
    if (!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.titleLabel.y, 12, 14)];
        _arrow.backgroundColor = [UIColor clearColor];
        _arrow.centerX = self.titleLabel.x - 10;
...
- (UIActivityIndicatorView *)activityIndicatorView {
    ...
    if(!_activityIndicatorView) {
        ...
        _activityIndicatorView.transform = CGAffineTransformMakeScale(0.75, 0.75);
...
- (UILabel *)titleLabel {
    ...
    if (!_titleLabel) {
        ...
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, 80, 15)];
        _titleLabel.centerX = floor(DEVICE_SCREEN_WIDTH * 0.5);
...
- (UILabel *)subtitleLabel {
    ...
    if (!_subtitleLabel) {
        ...
        _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.titleLabel.frame) + 10, 120, 14)];
        _subtitleLabel.centerX = floor(DEVICE_SCREEN_WIDTH * 0.5);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    self.showsDateLabel = YES;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"更新于: %@",), newLastUpdatedDate ? [self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"暂未更新",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"更新于: %@",), self.lastUpdatedDate ? [newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"暂未更新",)];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    UIImage * arrowImage = [UIImage imageNamed:@"pull_to_refresh_arrow"];
    [arrowImage drawInRect:CGRectMake(0, 0, 12, 14)];
